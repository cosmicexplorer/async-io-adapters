...
cfg_if! {
  if #[cfg(not(feature = "tokio"))] {
    ...
    enum SeekState {
      ...
      Start(std::io::SeekFrom),
      /// Waiting for completion of the first poll_complete in the
      /// `n.checked_sub(remainder).is_none()` branch.
...
impl<R> BufReader<R> {
  ...
  ///
  /// It is inadvisable to directly read from the underlying reader.
  pub fn get_pin_mut(self: Pin<&mut Self>) -> Pin<&mut R> { self.project().inner }
...
mod futures {
  ...
  impl<R: AsyncRead> AsyncRead for BufReader<R> {
    fn poll_read(
      ...
      let rem: &[u8] = ready!(self.as_mut().poll_fill_buf(cx))?;
      let amt: usize = std::cmp::min(rem.len(), buf.len());
  ...
  impl<R: AsyncRead> AsyncBufRead for BufReader<R> {
    fn poll_fill_buf(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<io::Result<&[u8]>> {
      ...
      if *me.pos >= *me.cap {
        ...
        let res = ready!(me.inner.poll_read(cx, me.buf))?;
        *me.cap = res;
        *me.pos = 0;
      }
  ...
  impl<R: AsyncSeek> AsyncSeek for BufReader<R> {
    fn poll_seek(
      ...
      let res = ready!(me.inner.poll_seek(cx, pos));
      /* Do discard_buffer() once the seek completes, as we can't trust our Seek
       * now. */
      *me.pos = 0;
      *me.cap = 0;
...
mod tokio {
    ...
    fn poll_complete(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<io::Result<u64>> {
      let res = match mem::replace(self.as_mut().project().seek_state, SeekState::Init) {
        ...
        SeekState::Start(SeekFrom::Current(n)) => {
          ...
          // absurd. But it's not out of the realm of possibility for some weird
          // underlying reader to support seeking by i64::MIN so we need to handle
          // underflow when subtracting remainder.
  ...
  impl<R: AsyncWrite> AsyncWrite for BufReader<R> {
    ...
    fn is_write_vectored(&self) -> bool { self.get_ref().is_write_vectored() }
